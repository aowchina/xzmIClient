...
@property (nonatomic ,strong) CRCarVINModel *vinModel;
/** 图片 */
@property (nonatomic, strong) NSString *cimag;
...
- (void)viewDidLoad {
    ...
    //    self.leftDataArr = @[@"品牌名称",@"车型名称",@"销售名称",@"车辆类型",@"17位的车架号VIN",@"发动机型号",@"功率/转速(Kw/R)",@"发动机喷射类型",@"燃油类型",@"变速器类型",@"发动机缸数",@"气缸形式",@"排量(L)",@"生产年份",@"安全气囊",@"座位数",@"车辆级别",@"车门数",@"车身形式",@"厂家名称",@"档位数",@"装备质量(KG)",@"组装厂"];
...
- (void)createTableView {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 64 + 50, kScreenWidth, kScreenHeight - 64 -50 - self.bottomHeight)];
...
- (void)buildBottomView {
    ...
    bottomBtn.frame = CGRectMake(0, kScreenHeight - 43, kScreenWidth, 43);
...
- (void)buildSearchView {
    ...
    self.carSearchView.searchBlock = ^{
        ...
        wself.bottomHeight = 0;
        wself.bottomBtn.hidden = YES;
        ...
        if (!code)
        {
            ...
            if (array.count > 0) {
                ...
                if (iType == 1) {// 标识为1有数据
                    ...
                    self.cimag = responseObjc[@"cimage"];
...
/*
 ...
 self.vinModel = [CRCarVINModel mj_objectWithKeyValues:responseObjc[@"che"][@"showapi_res_body"]];
...
- (void)clickSubmitBtnAction {
    ...
    /** 赋值 */
    CRCarDetailModel *model = [[CRCarDetailModel alloc] init];
    
    model.popType = @"VIN";
    model.Brand = self.vinModel.Brand;
    model.Series = self.vinModel.Series;
    model.SalesName = self.vinModel.SalesName;
    model.ProducedYear = self.vinModel.ProducedYear;
    model.vin = self.vinModel.vin;
    
    
    model.cimage = self.cimag;
    
    accessoriesSubmmitC.carModel = model;
