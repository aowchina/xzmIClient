...
#pragma a 消息显示用户昵称和头像
    ...
    if (model.isSender == YES) {//本人发的
        ...
        model.avatarImage = kImage(@"EaseUIResource.bundle/user");
//        model.avatarURLPath = @"http://a2.att.hudong.com/13/37/300533991095135084372975628.jpg";//头像网络地址
        return model;
    }
*/
...
- (NSArray*)emotionFormessageViewController:(EaseMessageViewController *)viewController
{
    ...
    for (NSString *name in names) {
        index++;
        EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:nil emotionId:[NSString stringWithFormat:@"em%d",(1000 + index)] emotionThumbnail:[NSString stringWithFormat:@"%@_cover",name] emotionOriginal:[NSString stringWithFormat:@"%@",name] emotionOriginalURL:@"" emotionType:EMEmotionGif];
