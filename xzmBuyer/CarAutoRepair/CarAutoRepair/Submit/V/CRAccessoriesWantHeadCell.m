...
- (void)reloadDataWithCarType:(CRCarDetailModel *)model andModel:(CRSubCarDetailModel *)subModel {
    ...
    if (model) {
        ...
        if ([model.popType isEqualToString:@"hand"]) {
            ...
            self.pingLabel.text = [NSString stringWithFormat:@"品牌：%@",model.Brand];
            self.xiLabel.text = [NSString stringWithFormat:@"车系：%@",model.Series];
            self.chexingLabel.text = [NSString stringWithFormat:@"车型：%@款 %@",model.ProducedYear,model.SalesName];
...
- (void)setCarDetailModel:(CRCarDetailModel *)carDetailModel {
    ...
    if ([carDetailModel.popType isEqualToString:@"hand"]) {
        ...
        self.pingLabel.text = [NSString stringWithFormat:@"品牌：%@",carDetailModel.Brand];
        self.xiLabel.text = [NSString stringWithFormat:@"车系：%@",carDetailModel.Series];
        self.chexingLabel.text = [NSString stringWithFormat:@"车型：%@",carDetailModel.SalesName];
