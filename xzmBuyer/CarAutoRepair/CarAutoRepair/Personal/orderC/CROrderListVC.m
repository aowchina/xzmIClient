...
- (IBAction)topBtnClick:(UIButton *)btn
{
    ...
    CGPoint ofset = self.bgScrollView.contentOffset;
    ofset.x = btn.tag * kScreenWidth;
    [_bgScrollView setContentOffset:ofset animated:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (orderDetailVC.orderType == 0)
    {
        
        makeSureVC.orderID = model.orderType == 0 ? model.qgorderid : model.orderid;
        makeSureVC.orderType = model.orderType;
        // 跳确认订单
        [self.navigationController pushViewController:makeSureVC animated:YES];
    }
    else
    {
        orderDetailVC.orderID = model.orderType == 0 ? model.qgorderid : model.orderid;
