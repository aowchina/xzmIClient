    ...
    [self.netWork asyncAFNPOST:initUrl Param:array Success:^(id responseObjc, NSError *codeErr) {
        ...
        if (!codeErr) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                EMError *error = [[EMClient sharedClient] loginWithUsername:[@"buy" stringByAppendingString:responseObjc[@"userid"]] password:responseObjc[@"tel"]];
                if (!error) {
...
- (IBAction)loginBtnAction {
    ...
    if (self.loginType == CRLoginYanZhengType) {
        ...
        if ([SuperHelper isEmpty:self.yanzmaLabel.text]) {
            ...
            [MBProgressHUD buildHudWithtitle:@"请填写密码" superView:self.view];
            return;
        }
        /** 验证验证码格式 */
    ...
    [self.netWork asyncAFNPOST:loginUrl Param:@[self.phoneLabel.text,self.yanzmaLabel.text] Success:^(id responseObjc, NSError *codeErr) {
        ...
        if (!codeErr) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                EMError *error = [[EMClient sharedClient] loginWithUsername:[@"buy" stringByAppendingString:responseObjc[@"userid"]] password:self.phoneLabel.text];
                if (!error) {
                ...
                /*
                    ...
                    EMError *error1 = [[EMClient sharedClient] registerWithUsername:[@"buy" stringByAppendingString:responseObjc[@"userid"]] password:self.phoneLabel.text];
            ...
            if (code == 10 || code == 11 || code == 12) {
                ...
                [MBProgressHUD buildHudWithtitle:@"手机号不正确" superView:self.view];
            } else if (code == 14 || code == 17) {
                [MBProgressHUD buildHudWithtitle:@"密码不正确" superView:self.view];
            } else if (code == 16) {
                [MBProgressHUD buildHudWithtitle:@"用户不存在" superView:self.view];
            } else if (code == 19) {
                [MBProgressHUD buildHudWithtitle:@"电话已经被注册" superView:self.view];
            } else if (code == 49) {
                [MBProgressHUD buildHudWithtitle:@"验证码不正确" superView:self.view];
