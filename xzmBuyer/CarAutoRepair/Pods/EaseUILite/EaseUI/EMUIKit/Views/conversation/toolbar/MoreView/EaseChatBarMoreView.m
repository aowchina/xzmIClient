...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    UILabel *photoLabel = [self createLabelWithText:@"照片" andFrame:CGRectMake(insets, 60, CHAT_BUTTON_SIZE, 25)];
    ...
    UILabel *locationLabel = [self createLabelWithText:@"位置" andFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 60, CHAT_BUTTON_SIZE, 25)];
    ...
    UILabel *takePicLabel = [self createLabelWithText:@"拍摄" andFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 60, CHAT_BUTTON_SIZE, 25)];
...
- (void)insertItemWithImage:(UIImage *)image highlightedImage:(UIImage *)highLightedImage title:(NSString *)title
{
    ...
    [moreButton setFrame:CGRectMake(page * CGRectGetWidth(self.frame) + insets * (col + 1) + CHAT_BUTTON_SIZE * col, INSETS + INSETS * 1 * row + (CHAT_BUTTON_SIZE + 25) * row, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    ...
    UILabel *moreLabel = [self createLabelWithText:title andFrame:CGRectMake(page * CGRectGetWidth(self.frame) + insets * (col + 1) + CHAT_BUTTON_SIZE * col, INSETS + INSETS * 1 * row + (CHAT_BUTTON_SIZE + 25) * row + 50, CHAT_BUTTON_SIZE ,25)];
...
- (UILabel *)createLabelWithText:(NSString *)text andFrame:(CGRect)rect {
