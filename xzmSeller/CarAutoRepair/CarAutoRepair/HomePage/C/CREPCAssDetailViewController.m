...
{
    CRSalePeijianController *salePeiJianVC = [[CRSalePeijianController alloc] init];
    /** 类型分别 */
    salePeiJianVC.peijianType = 7;
    
    salePeiJianVC.nameStr = self.nameLabel.text;
    salePeiJianVC.bianmaStr = [self.oemLabel.text stringByReplacingOccurrencesOfString:@"OEM号：" withString:@""];
    
    NSMutableArray *array = [NSMutableArray array];
    
    for (CRCarDetailModel *carModel in self.dataArr) {
        
        [array addObject:carModel.carid];
    }
    
    salePeiJianVC.carArray = array;
    
    [self.navigationController pushViewController:salePeiJianVC animated:YES];
}
