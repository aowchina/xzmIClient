...
@property (nonatomic, assign) NSInteger renzType;

@property (nonatomic, assign) NSInteger btn_tag;

@property (nonatomic, strong) CRCerModel *cerModel;
@end
...
- (void)viewDidLoad {
    ...
    self.imageArr = [NSMutableArray arrayWithArray:@[@" ",@" ",@" ",@" ",@" "]];
...
- (void)requestData {
    ...
    NSString *Main_Url = [self.baseUrl stringByAppendingString:@"user/AuthInfo.php"];
    ...
    [self.netWork asyncAFNPOST:Main_Url Param:arr Success:^(id responseObjc, NSError *codeErr) {
        ...
        if (!code)
        {
            ...
            if (responseObjc) {
                ...
                self.cerModel = [CRCerModel mj_objectWithKeyValues:responseObjc];
                
                [self.tableView reloadData];
                
                self.footView.cerModel = self.cerModel;
                
                self.renzType = [self.cerModel.status integerValue];
                
                if (self.renzType == 0) {
                    
                    [self.btn setTitle:@"审核中.." forState:UIControlStateNormal];
                    self.btn.userInteractionEnabled = NO;
                    
                } else if (self.renzType == 1) {
                    
                    [self.btn setTitle:@"审核通过" forState:UIControlStateNormal];
                    self.btn.userInteractionEnabled = NO;
                    
                } else if (self.renzType == 2) {
                    
                    [self.btn setTitle:@"审核不通过，重新审核" forState:UIControlStateNormal];
                ...
                self.cerModel = [[CRCerModel alloc] init];
                
                self.cerModel.status = @(1000).stringValue;
        ...
        {
            [MBProgressHUD alertHUDInView:self.view Text:@"请稍后重试"];
        }
        else
        {
...
- (void)buildBottomView {
    ...
    self.btn.frame = CGRectMake(0, kScreenHeight - 44 - IS_HOTSPOT_HEIGHT, kScreenWidth, 44);
    [self.btn setBackgroundImage:kImage(@"address_wdqb_tixiananniu") forState:UIControlStateNormal];
    [self.btn setTitle:@"确定申请" forState:UIControlStateNormal];
    [self.btn addTarget:self action:@selector(clickSubmitBtnAction) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell reloadDataWithModel:self.cerModel andIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 3) {
        ...
        if (self.renzType == 1000 || self.renzType == 2) {
...
- (void)clickSubmitBtnAction {
    ...
    if (self.cerModel.skill.length == 0)
    {
        [MBProgressHUD alertHUDInView:self.view Text:@"请填写范围"];
        return;
    }
    if (self.cerModel.major.length == 0)
    {
        [MBProgressHUD alertHUDInView:self.view Text:@"请选择专长"];
        return;
    }
    
    for (id objc in self.imageArr) {
        
        if ([objc isKindOfClass:[NSString class]]) {
            
            if (self.renzType == 2) {
                [MBProgressHUD alertHUDInView:self.view Text:@"请重新上传全部照片信息"];
            } else {
                [MBProgressHUD alertHUDInView:self.view Text:@"请上传全部照片信息"];
            }
            return;
        }
    ...
    [self.netWork asyncPhotoListForPersonWithImageArray:self.imageArr parameterArray:arr url:Main_Url success:^(id responseObjc) {
        ...
        if (!code)
        {
            ...
            [MBProgressHUD alertHUDInView:self.view Text:@"申请成功，请等待审核通过"];
        }
        else if (code == 13)
        {
